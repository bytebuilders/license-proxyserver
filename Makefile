...
PRODUCT_NAME       := license-proxyserver
ENFORCE_LICENSE    ?=

GO_PKG   := go.bytebuilders.dev
REPO     := $(notdir $(shell pwd))
BIN      := license-proxyserver
COMPRESS ?= no
...
# 	rm -rf ./apis/proxyserver/v1beta1/zz_generated.conversion.go
# 	@docker run --rm                                            \
# 		-u $$(id -u):$$(id -g)                                    \
# 		-v /tmp:/.cache                                           \
# 		-v $$(pwd):$(DOCKER_REPO_ROOT)                            \
...
# 			--input-dirs $(GO_PKG)/$(REPO)/apis/proxyserver/v1beta1 \
...
uninstall:
	@cd ../installer; \
	helm uninstall license-proxyserver --namespace=$(KUBE_NAMESPACE) || true
...
.PHONY: run
run:
	go run -mod=vendor ./cmd/license-proxyserver run \
		--v=3 \
		--secure-port=8443 \
